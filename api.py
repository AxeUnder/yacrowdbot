            ...
            async with session.get(API_URL_POST, headers=HEADERS) as response:
                if response.status == 200:
                    try:
                        ...
                        logging.error(f'Ошибка чтения ответа API постов: {error}')
                        return []
                else:
                    logging.error(f'Ошибка получения постов: {response.status}')
                    return []
        except aiohttp.ClientError as error:
            logging.error(f'Ошибка сети при получении постов: {error}')
            return []
...
async def get_users():
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            logging.error(f'Ошибка сети при получении пользователей: {error}')
            return []
            ...
            async with session.get(f'{API_URL_USER}/{chat_id}', headers=HEADERS) as response:
                if response.status == 200:
                    try:
                        ...
                        logging.error(f'Ошибка чтения данных пользователя: {error}')
                        return {}
                else:
                    logging.error(f'Ошибка получения данных пользователя: {response.status}')
                    return {}
        except aiohttp.ClientError as error:
            logging.error(f'Ошибка сети при получении данных пользователя: {error}')
            return {}
            ...
            async with session.post(API_URL_USER, headers=HEADERS, json=data) as response:
                if response.status in (200, 201):
                    try:
                        ...
                        logging.error(f'Ошибка чтения ответа API при сохранении пользователя: {error}')
                        return {}
                else:
                    response_text = await response.text()
                    logging.error(f'Ошибка сохранения данных пользователя: {response.status}, ответ: {response_text}')
                    return {}
        except aiohttp.ClientError as error:
            logging.error(f'Ошибка сети при сохранении данных пользователя: {error}')
            return {}
            ...
            async with session.patch(f'{API_URL_USER}/{chat_id}', headers=HEADERS, json=data) as response:
                ...
                if response.status == 200:
                    try:
                        ...
                        logging.error(f'Ошибка чтения ответа API при обновлении пользователя: {error}')
                        return {}
                else:
                    logging.error(f'Ошибка обновления данных пользователя: {response.status}, ответ: {response_text}')
                    return {}
        except aiohttp.ClientError as error:
            logging.error(f'Ошибка сети при обновлении данных пользователя: {error}')
            return {}
        except Exception as error:
            logging.error(f'Неожиданная ошибка при обновлении данных пользователя: {error}')
            return {}
